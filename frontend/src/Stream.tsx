        ...
        <>
            ...
            <aside class="flex flex-col mlg:max-h[calc(100vh-160px)] mmd:w-[350px] mmd:w-[100%] mmd:flex-1 mmd:min-h-[400px]" aria-label="Sidebar">
                ...
                <div class="flex align m-2 min-w-0">
                    <input class="text-primary input input-primary input-bordered rounded-l rounded-r-none flex-1 min-w-0" ref={input} onkeydown={send} id="shitpost" type="text" placeholder="...">
                    </input>
                    <button class="btn-primary py-2 px-4 font-light" innerText="💌" onTouchEnd={send} onclick={send}></button>
                    <button class="btn-secondary py-2 px-4 rounded-r" innerText="🔁" onTouchEnd={send} onclick={chatService().reconnect}></button>
                </div>
            </aside>
        </>
    );
